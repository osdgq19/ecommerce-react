...
export default function OrderScreen(props) {
    ...
    useEffect(() => {
        ...
        if (!order || successPay || (order && order.cabeza[0].id_order != orderId)) {
                        ...
                        <ul>
                            <li>
                                <div className="card card-body">
                                    ...
                                    <p>
                                        ...
                                        <strong>Direccción: </strong> {order.cabeza[0].direction} {order.cabeza[0].address} ,
                            ...
                            <li>
                                <div className="card card-body">
                                    <h2>Pago</h2>
                                    <p>
                                        <strong>Método de Pago: </strong> {order.cabeza[0].name} <br></br>

                                    </p>
                                    {!order.cabeza[0]?.capture && <div>
                                        <label htmlFor="image">Ingrese Capture {' '}</label>
                                        <input type="file" id="image"
                                            placeholder="Ingrese Capture"
                                            required onChange={handleImage}></input>
                                    </div>
                                    } {order.cabeza[0].isPaid === "" ? <p>Orden sin pagar</p> : <p>Orden pagada el: {order.cabeza[0].paidAt} </p>}
                                    {order.cabeza[0].isPaid === "" ? <p>Capture no registrado</p> : <a href={order.cabeza[0]?.capture}>Capture: {order.cabeza[0]?.capture} </a>}
                                    {console.log(order)}
                                </div>
                            ...
                            <ul>
                                ...
                                <li>
                                    <div className="row">
                                        ...
                                        <strong> <div>${order.cabeza[0].total}</div></strong>
                                            ...
                                            <li>
                                                {
                                                    <>
                                                        ...
                                                        {<button className="primary block" onClick={successPaymentHandler} disabled={!imageFile} >Pagar</button>}

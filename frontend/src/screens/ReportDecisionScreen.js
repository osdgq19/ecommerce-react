        ...
        <div>
...

            {
                ...
                loading ? <LoadingBox></LoadingBox> : error ? <MessageBox variant="danger">{error}</MessageBox>
                    : tipoReporte==="Producto" ? (<table className="table">
                        <thead>
                            <tr>
                                <th>ID</th>
                        ...
                        <tbody>
                            {console.log(reportInfo)}
                            {
                             ...
                             reportInfo?.ordenes.map((report) => {return(
                        ...
                        <tbody>
                            {
                             ...
                             reportInfo?.ordenes.map((report) => {return(
                                <tr key={report.id_order}>
                                    ...
                                    <td><button type="button" className="small" onClick={() => props.history.push(`/order/${report.id_order}`)}>Details</button></td>
            ...
            tipoReporte==="Ganancias"?<h1>Ganancias Totales desde {fechaInicio} hasta {fechaFinal}: {' '} {reportInfo?.ganancia[0].ganancias}$ </h1>
            
            : tipoReporte==="Entregas" ? <h1>Nro total de Entregas desde {fechaInicio} hasta {fechaFinal}: {' '} </h1>

            : tipoReporte==="Ordenes" ? <h1>Nro total de Ordenes desde {fechaInicio} hasta {fechaFinal}: {' '} </h1>
            
            : tipoReporte==="Producto" ? <h1>Producto mas vendido desde {fechaInicio} hasta {fechaFinal}: {' '} </h1>
            
            : <h1>Nro total de Ordenes Cobradas desde {fechaInicio} hasta {fechaFinal}: {' '}  </h1>

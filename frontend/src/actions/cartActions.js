...
import { CART_ADD_ITEM, CART_REMOVE_ITEM, CART_SAVE_PAYMENT_METHOD, CART_SAVE_SHIPPING_ADDRESS, STATE_FAIL, STATE_REQUEST, STATE_SUCCESS } from "../constants/cartConstants"



export const addToCart = (productID, qty, user, mensaje) => async (dispatch, getState) => {
          console.log(mensaje)
            const dato = await Axios.post('/api/cartlist/add', { user, productID, qty, mensaje })  
...
export const removeFromCart = (productID, userID) => async (dispatch, getState) => {
      ...
      //localStorage.setItem('cartItems', JSON.stringify(getState().cart.cartItems))
      try {
            const { data } = await Axios.post('/api/cartlist/remove', { productID, userID })
...
export const saveShippingAddress = (data) => async (dispatch) => {
      ...
      console.log(estado)
      const idShipping = await Axios.post('/api/orders/shipping', { fullName, address, estado })

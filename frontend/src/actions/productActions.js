    ...
    dispatch({ type: PRODUCT_ADD_REQUEST })
      try {console.log(datos)
          ...
          dispatch({
            ...
            type: PRODUCT_ADD_SUCCESS, payload: data
      ...
      localStorage.setItem('mineProds', JSON.stringify(getState().myProds.mineProds))
}
...
export const addToMyProd = (user) => async (dispatch, getState) => {
    console.log(user)
    const { data } = await Axios.post('/api/products/myprods', {idusers: user.idusers})
    console.log(data)
  data.map( c =>
    dispatch({
...
export const removeMyProd = (productID, user) => async(dispatch, getState) => {
    ...
    const { data } = await Axios.post('/api/products/deletemyprod', {productID,user})
   
    dispatch({ type: MINE_REMOVE_ITEM, payload: data });

    localStorage.removeItem('mineProds', JSON.stringify(getState().myProds.mineProds))
...
export const updateMyProd = (datos) => async(dispatch, getState) => {
    ...
    try {console.log(datos)
        ...
        dispatch({
          ...
          type: PRODUCT_UPD_SUCCESS, payload: data

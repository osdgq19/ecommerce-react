...
export const productUpdReducer = (
  state = {loading: true, product: {}},
  action,
) => {
  switch (action.type) {
    case PRODUCT_UPD_REQUEST:
      return {loading: true}

    case PRODUCT_UPD_SUCCESS:
      return {loading: false, product: action.payload}

    case PRODUCT_UPD_FAIL:
      return {loading: false, error: action.payload}
...
export const myProdReducer = (state = {mineProds: []}, action) => {
  switch (action.type) {
    ...
    case MINE_REMOVE_ITEM:
      return {
        ...state,
        mineProds: state.mineProds.filter(x => x.product !== action.payload),
      }
